...
std::string get_rnd_key(size_t r) {
  std::mt19937_64 mt(r);
  char buffer[65];
  snprintf(buffer + 0, 17, "%016llX", (long long)mt());
  snprintf(buffer + 16, 17, "%016llX", (long long)mt());
  snprintf(buffer + 32, 17, "%016llX", (long long)mt());
  snprintf(buffer + 48, 17, "%016llX", (long long)mt());
  // uint64_t v = mt();
