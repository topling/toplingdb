// This file is nasty, but I need it for testing, just let it works - rockeet@
...
#else
    std::set<std::string> rand_key_0;
    std::set<std::string> rand_key_1;
#endif
    if (mode == 0) {
...
#define patricia_WriteBatchEntryIndexFactory() nullptr
    ...
    for (uint64_t count = atomic_count;; count = ++atomic_count) {
      ...
      if (std::uniform_int_distribution<uint64_t>(0, 1 << 10)(mt) == 0) {
        ...
        if (snapshot_mutex.try_lock()) {
          auto del_snapshot = [&](const rocksdb::Snapshot *snap) {
...
#else
    repo_.ImportJsonFile(FLAGS_json);
    s = repo_.OpenDB(&db_mcf);
    if (!s.ok()) {
      fprintf(stderr, "FATAL: repo_.OpenDB() = %s\n", s.ToString().c_str());
      exit(1);
    }
