...
std::string errnoStr(int err) {
  ...
  void stdstr_resize_no_init(std::string& str, std::size_t sz) {
    str.resize_and_overwrite(sz, [](char*, size_t sz) { return sz; });
  }
  ...
  void stdstr_resize_no_init(std::string& str, std::size_t sz) { str.resize(sz); }
#else
  ...
  inline void stdstr_resize_no_init(std::string& str, std::size_t sz) {
#if defined(__GLIBCXX__) || defined(_LIBCPP_VERSION) || defined(_MSVC_STL_VERSION)
      str.reserve(sz);
