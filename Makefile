...
#-----------------------------------------------
BMI2 ?= 0
COMPILER=$(shell ${CXX} terark-tools/detect-compiler.cpp -o a && ./a && rm -f a a.exe)
...
EXTRA_LDFLAGS += -L${TerocksDir}/lib
ifeq (${DEBUG_LEVEL}, 0)
...
AM_LINK = $(AM_V_CCLD)$(CXX) $^ $(EXEC_LDFLAGS) -o $@ -lrocksdb ${EXTRA_LDFLAGS} $(LDFLAGS) $(COVERAGEFLAGS)
